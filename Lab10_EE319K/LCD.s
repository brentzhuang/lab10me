...
step1
	LDR R1, =SSI0_SR_R
	LDR R2, [R1]
	AND R3, R2, #SSI_SR_BSY
	CMP R3, #SSI_SR_BSY
    BEQ step1
	

    LDR R1, =DC			;R1 = 0x40004100 (D/C)
	LDR R2, =DC_COMMAND	;R2 = 0			
	STR R2, [R1]		;DC = 0 (cleared PA6)
	
	LDR R1, =SSI0_DR_R
	STRB R0, [R1]		;write input R0 into SSI0_DR_R
...
loop
	LDR R1, =SSI0_SR_R
	LDR R2, [R1]
	AND R3, R2, #SSI_SR_TNF
    CMP R3, #SSI_SR_TNF
	BMI loop
	
	LDR R1, =DC			;R1 = 0x40004100 (D/C)
	LDR R2, =DC_DATA	;R2 = 0x40	
	STR R2, [R1]		;DC = 0 (cleared PA6)
